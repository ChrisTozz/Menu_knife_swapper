...
{
	RegConsoleCmd("knife", OnCommandKnife);
...
public Action OnCommandKnife(int client, int args)
{
	...
	{
		ReplyToCommand(client, "[SM] You dont have permission to access this command. You need to join FPSUltra steam group https://steamcommunity.com/groups/FPSUltra");
		return Plugin_Handled;
	}
	
	int clientWithKnife = 0;
	char name[MAX_NAME_LENGTH], str[32];
	
	for (int i = 1; i <= MaxClients; i++)
	{			
		if(IsClientInGame(i) && !IsFakeClient(i) && PlayerHasKnife(i) && test(i, i, i) && !(i == client))
		{
			GetClientName(i, name, sizeof(name));
			IntToString(i, str, sizeof(name));

			
			StrCat(name, sizeof(name), "'s knife");
			
		
			int targetknife = GetPlayerWeaponSlot(i, CS_SLOT_KNIFE);
			if (!(targetknife == -1))
			{
			menu.AddItem(str, name, ITEMDRAW_DEFAULT);
			clientWithKnife++;
			}
		}
	}
	
	if (clientWithKnife == 0)
	{
		ReplyToCommand(client, "[SM] No player with custom knife found on server");
		return Plugin_Handled;
	}
		...
		case MenuAction_Select:
		{
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				if(IsClientInGame(i) && !IsFakeClient(i))
				{
					...
					if (StrEqual(item, str))
					{
						char name[MAX_NAME_LENGTH];
						GetClientName(i, name, sizeof(name));
						GivePlayerKnife(i, param1, param1);
						g_iTakeKnifeUserIds[param1] = GetClientUserId(i);
					}
				}
			}
		}	
...
stock bool PlayerHasCustomKnife(int player)
{
	g_bBlockItemEquip[player] = true; // paintkits fix
	int newknife = GivePlayerItem(player, "weapon_knife");
	g_bBlockItemEquip[player] = false; // paintkits fix
...
stock bool test(int client, int target, int commandclient, bool displaymessage=true)
{
	if (!IsClientInGame(client))
		return false;
